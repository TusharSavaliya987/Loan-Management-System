...
export function LoanCard({ loan, customer }: LoanCardProps) {
  ...
  const nextPayment = loan.interestPayments.find(payment => payment.status === "pending");
    ...
    <Card>
      ...
      <CardContent className="pb-2">
        <div className="grid gap-2">
          ...
          <div className="flex justify-between">
            ...
            <span className="capitalize">{loan.interestFrequency}</span>
          </div>
          {nextPayment && (
            <div className="mt-2 pt-2 border-t">
              <div className="flex justify-between">
                <span className="text-sm font-medium">Next payment</span>
                <span className="font-medium">{format(parseISO(nextPayment.dueDate), "PPP")}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Amount</span>
                <span>₹{nextPayment.amount.toLocaleString()}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <CardFooter className="flex gap-2">
        ...
        <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <Button variant="outline" onClick={() => setEditDialogOpen(true)} className="flex-1">
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <EditLoanForm loan={loan} onSuccess={() => setEditDialogOpen(false)} />
          </DialogContent>
        </Dialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete this loan? The loan will be moved to trash and can be restored within 100 days.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
