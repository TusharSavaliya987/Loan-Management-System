...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  if (!firebaseAdminAuth || !db) {
    console.error("API Error: Firebase Admin SDK not initialized for PATCH /api/loans/restore/[id]. Check server logs.");
  ...
  try {
    ...
    if (!loanId) {
      return NextResponse.json({ message: 'Loan ID is required for restore.' }, { status: 400 });
    }
    ...
    if (!loanDoc.exists) {
      return NextResponse.json({ message: 'Loan not found for restore.' }, { status: 404 });
    }

    if (loanDoc.data()?.userId !== userId) {
      return NextResponse.json({ message: 'Unauthorized to restore this loan.' }, { status: 403 });
    }

    if (loanDoc.data()?.status !== 'deleted') {
      return NextResponse.json({ message: 'Loan is not deleted, cannot restore.' }, { status: 400 });
    }
    ...
    console.error('Error in PATCH /api/loans/restore/[id]:', error);
